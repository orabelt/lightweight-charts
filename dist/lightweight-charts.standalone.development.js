/*!
 * @license
 * TradingView Lightweight Charts v4.0.0-dev+202207121504
...
(function () {
            ...
            for (let i = visibleRange.from + 1; i < visibleRange.to; ++i) {
                ...
                switch (lineType) {
                    case 0 /* Simple */:
                        ...
                        if (!isNaN(currItem._internal_price) && i > 1 && !isNaN(items[i - 1]._internal_price)) {
                            ctx.lineTo(currItem._internal_x, currItem._internal_y);
                        }
                        else {
    ...
    function fillSizeAndY(rendererItem, marker, seriesData, offsets, textHeight, shapeMargin, priceScale, timeScale, firstValue) {
        ...
        switch (marker.position) {
            case 'inBar': {
                ...
                if (((_a = rendererItem._internal_text) === null || _a === void 0 ? void 0 : _a._internal_content) === '#{formula:priceToCoordinate(id)}' && isNumber(marker.id)) {
                    // const inBarPriceId = isNumber(marker.id) ? marker.id : NaN;
                    rendererItem._internal_y = priceScale._internal_priceToCoordinate(marker.id, firstValue);
                    rendererItem._internal_text._internal_content = '';
                }
                return;
            }
            case 'aboveBar': {
    ...
    class ChartModel {
        ...
        _internal_setAndSaveCurrentPosition(x, y, pane) {
            ...
            this._private__crosshairMoved._internal_fire(this._private__crosshair._internal_appliedIndex(), { x, y, _internal_paneIndex: paneIndex });
        }
